...
for (const placed of placedTiles) {
  this.board.removeTile(placed.row, placed.col)  // ❌ Tiles removed!
}

// Validate
const invalidWords = this.wordValidator.getInvalidWords([mainWord])  // ❌ Reads null tiles!
```
...
while (startCol > 0 && !this.isEmpty(row, startCol - 1)) {
  startCol--  // ❌ Continues through blockers!
}
```
...
- ✅ Minimum 4-letter length
- ✅ Forms a valid line (horizontal/vertical)
- ✅ Connects to existing tiles
- ✅ First move touches center
- ❌ ~~Dictionary validation~~ (removed - only via challenge)
...
- **Confirmation Dialog:**
  ...
  ```
- **Result Dialogs:**
  - **Success:** "✅ Challenge successful! The word 'X' is invalid. Reason: Y. The move has been undone."
  - **Failure:** "❌ Challenge failed! The word 'X' is valid. You have been penalized 3 minutes."
...
Opponent can challenge
  ↓
IF challenged → Validate DICTIONARY
  ↓
Success: Undo move
...
**Incorrect Flow:**
```
Place tiles → Mark premium squares as used (isUsed = true)
  ↓
Calculate score → Check !isUsed → Always false → No multipliers applied ❌
```
...
if (tile && square.premiumField && !square.isUsed) {
  square.isUsed = true  // ❌ Marked too early!
}
...
boardInstance.markSquaresAsUsed(placedTiles)  // ✅ Marked at correct time
```

**Correct Flow:**
```
Place tiles → Premium squares remain isUsed = false
  ↓
Calculate score → Check !isUsed → True → Multipliers applied ✅
  ↓
Mark premium squares as used → Prevents double-application on future turns
```

**Debug Output:**
With existing debug logging in `ScoreCalculator.ts`, you'll now see:
```
💰 Score calculation for tile: {
  position: '(8,4)',
  letter: 'K',
  value: 2,
  isNew: true,
  premiumField: 'DOUBLE_LETTER',
  isUsed: false,  // ✅ Now false during scoring
  willApplyMultiplier: true  // ✅ Now true!
}
  ✅ Applied DOUBLE_LETTER: 2 × 2 = 4
💰 Final score for word "KUĆA": { baseScore: 11, wordMultiplier: 1, finalScore: 11 }
✅ Marked (8,4) DOUBLE_LETTER as used
```
...
- [x] Place word vertically using letter from horizontal word → No "Word too short" error
- [x] Place word horizontally using letter from vertical word → No "Word too short" error
- [x] Blocker tiles prevent word extension → Words stop at blockers
- [x] Invalid words accepted without challenge → Words placed successfully
- [x] Challenge valid word → Challenger loses 3 minutes
- [ ] Challenge invalid word → Move undone, tiles returned (TODO)
...
**What Was Fixed:**
1. ✅ Word validation bug causing false "Word too short" errors (3 root causes)
2. ✅ getTilesInLine now respects blocker tile boundaries
3. ✅ Only main word validated (not cross-words)
4. ✅ Word extraction happens before tile removal
5. ✅ Premium field multipliers now applied correctly to scores
6. ✅ Premium squares timing bug fixed (isUsed flag set after scoring)

**What Was Added:**
1. ✅ Challenge system (no automatic dictionary validation)
2. ✅ Challenge button UI with confirmation dialogs
3. ✅ 3-minute time penalty for failed challenges
4. ✅ lastPlayedWord state tracking
5. ✅ Debug logging for move validation
6. ✅ Debug logging for score calculation
7. ✅ Board.markSquaresAsUsed() method

**What Still Needs Work:**
1. ❌ Full move undo when challenge succeeds
2. ❌ Remove debug logs from production build

**Build Status:** ✅ Successful (187.70 KB)
...
- `src/game-engine/Board.ts` - Blocker boundary detection in getTilesInLine, markSquaresAsUsed method, removed auto-marking from setTile
- `src/game-engine/ScoreCalculator.ts` - Added debug logging for score calculation

### State Management
- `src/store/gameStore.ts` - Challenge mechanism, lastPlayedWord state, time penalty, markSquaresAsUsed call after scoring
...
- **Added:** ~170 lines
- **Modified:** ~80 lines
- **Deleted:** ~40 lines (removed cross-word validation, auto-marking logic)

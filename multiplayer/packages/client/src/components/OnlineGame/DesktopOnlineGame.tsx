/**
 * Desktop Online Game Component
 *
 * Main game screen for online multiplayer (desktop layout).
 *
 * This component:
 * - Shows the game board and tiles in 3-column layout
 * - Handles online game actions via WebSocket
 * - Shows connection status
 * - Displays opponent information
 * - Handles game completion
 *
 * Layout: [Scoresheets + Chat] [Board + Rack] [Score Panel + Controls]
 *
 * For mobile devices, use MobileOnlineGame instead.
 */
...
export function DesktopOnlineGame() {
  ...
  const handlePlayWord = () => {
    Logger.debug('[DesktopOnlineGame] Play Word clicked, selectedTiles:', selectedTiles.length)
    if (selectedTiles.length === 0) {
      return
    }
